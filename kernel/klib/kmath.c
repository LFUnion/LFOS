...
double continuous_fraction_root(const double minus, const double radicand,
                                uint8_t max) {
    if(max ==0)
        return 1;

    --max;
    double rvalue = 1/(((minus + minus) + continuous_fraction_root(minus, radicand,
                        max))/(radicand - minus));
    return rvalue;
}
...
double root (const double radicand, double exponent) {
    ...
    for (unsigned int c = 0; c<recursion; ++c)
        rvalue = ((exponent_low)* pow(rvalue,
                                      exponent) + radicand)/(exponent * pow(rvalue,exponent_low));
